...
public class BData {
    ...
    public List<Budget> filterByDate(SessionDTO filter)
    {
    ...
    private List<Budget> findByQH(LocalDate d,int m,int inc) {
        LocalDate stop = DUtil.setMonth(d,m).plusMonths(inc);

        return doFilter(d,stop);
    }

    private List<Budget> findSpecific(LocalDate d) {
    ...
    private List<Budget> findByYear(LocalDate d) {
    ...
    private List<Budget> findByMonth(LocalDate d) {
        int month = d.getMonth().getValue();
        LocalDate start = DUtil.first();
        LocalDate stop = DUtil.stopNow();
        List<Budget> data = doFilter(start,stop);
