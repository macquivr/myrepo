...
public class BSData {
    ...
    public BSData(BudgetsRepository l)
    {
        repository = l;
        dates = new StartStop();
    }
    ...
    public List<Budgets> filterByDate(SessionDTO filter)
    {
        ...
        return new Vector<Budgets>();
    }

    private List<Budgets> doFilter(LocalDate start, LocalDate stop) {
        dates.setStart(start);
        dates.setStop(stop);
       
        return repository.findAllByBdateBetweenOrderByBdateAsc(start, stop);
    }
    private List<Budgets> findByQH(LocalDate d,int m,int inc) {
        LocalDate stop = DUtil.setMonth(d,m).plusMonths(inc);

        return doFilter(d,stop);
    }

    private List<Budgets> findSpecific(LocalDate d) {
    ...
    private List<Budgets> findByYear(LocalDate d) {
    ...
    private List<Budgets> findByMonth(LocalDate d) {
        int month = d.getMonth().getValue();
        LocalDate start = DUtil.first();
        LocalDate stop = DUtil.stopNow();
        List<Budgets> data = doFilter(start,stop);

        return data.stream().filter(it -> (it.getBdate().getMonth().getValue() == month)).collect(Collectors.toList());
    }
}
